...
if(!isset($_SESSION["user"])){
  ...
  if($result->num_rows > 0 ){
    ...
    $objPHPExcel->getProperties()->setCreator("OPLEVER") //Autor
               ->setLastModifiedBy("OPLEVER") //Ultimo usuario que lo modificó
 ...
 while ($fila=mysqli_fetch_row($result)) {
  ...
  foreach ($fila as $key => $value) {
      ...
      $objPHPExcel->setActiveSheetIndex(0)
        ...
        }else{if($value!=null){$objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue($filas[$key-1].$i, utf8_encode($value));}
          }}
 }
